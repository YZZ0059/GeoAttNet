...
def train(model, train_loader, val_loader, criterion, optimizer, device, num_epochs=100, scheduler=None, patience=15, start_time=None):
    ...
    for epoch in range(num_epochs):
        ...
        if val_f1 > best_val_f1:
            ...
            safe_print(f'✓ best model (F1: {val_f1:.4f})')
        else:
            patience_counter += 1  
    if start_time:
        from datetime import datetime
...
def main():
    ...
    parser = argparse.ArgumentParser(description='GeoAttNet-Base')
    ...
    model = DeepUNet(in_channels=15, num_classes=1, dropout_rate=0.2, use_attention=False).to(device)
...
def create_text_report(final_metrics, history, result_dir):
    ...
    report_lines.append("GeoAttNet-Base Training Completion Report")
    report_lines.append("=" * 60)
