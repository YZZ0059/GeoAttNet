...
def calculate_metrics(all_preds, all_targets, threshold=0.5, verbose=True):
    ...
    if verbose:
        safe_print(f"\Use the optimal threshold {optimal_threshold:.3f} Evaluation results:")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='GeoAttNet-CBAM')
...
def create_text_report(final_metrics, history, result_dir):
    ...
    report_lines.append("GeoAttNet-CBAM Training Completion Report")
    report_lines.append("=" * 60)
